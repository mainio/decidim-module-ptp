...
shared_context "with scopes" do
  ...
  let!(:first_postals) do
    [].tap do |postals|
      5.times do |i|
        code = (10_000 + i).to_s
        postals << create(:scope, name: { en: code }, code: "FIRST_#{code}", parent: subscopes[0], organization:)
      end
    end
  end
  let!(:second_postals) do
    [].tap do |postals|
      7.times do |i|
        code = (10_010 + i).to_s
        postals << create(:scope, name: { en: code }, code: "SECOND_#{code}", parent: subscopes[1], organization:)
      end
    end
  end
  let!(:third_postals) do
    [].tap do |postals|
      8.times do |i|
        code = (10_020 + i).to_s
        postals << create(:scope, name: { en: code }, code: "THIRD_#{code}", parent: subscopes[2], organization:)
      end
    end
  end
end
